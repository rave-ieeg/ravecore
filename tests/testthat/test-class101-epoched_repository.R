...
test_that("RAVESubjectEpochRepository", {
  ...
  repository_id <- rand_string(4)
  repo_old <- prepare_subject_with_epoch_legacy(
  ...
  repo_new0 <- RAVESubjectEpochRepository$new(
    subject = "demo/DemoSubject",
    electrodes = 13:16,
    reference_name = "default",
    epoch_name = "auditory_onset",
  ...
  testthat::expect_true(inherits(repo_new, "rave_prepare_with_epoch"))

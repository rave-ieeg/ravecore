...
def coregistrate_image(
  ...
  if precenter_source:
    ...
    ct_sform2[1, 3] = 0
    ct_sform2[2, 3] = 0
    ct_cmap_translate = numpy.matmul(ct_sform2, numpy.array([*ctimg.shape,2.]) / 2.) 
  ...
  registration_results = ct_to_mri_reg.optimize(reg_type, optimizer=optimizer, xtol=1e-8, ftol=1e-8)
  aff = registration_results.as_affine()

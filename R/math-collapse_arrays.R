...
#' @param x R array, \code{\link[filearray]{FileArray-class}}, or other objects
...
collapse2.FileArray <- function(x, keep, method = c("mean", "sum"), ...){
  ...
  if(ndims %in% keep){
    ...
    re <- ravetools::collapse(re, seq_along(keep), average = is_mean)
  }
...
#' @rdname collapse2
#' @export
collapse2.RAVEFileArray <- function(x, keep, method = c("mean", "sum"), ...) {
  method <- match.arg(method)
  collapse2.FileArray(x$`@impl`, keep = keep, method = method, ...)
}
...
collapse_power.array <- function(x, analysis_index_cubes){
  ...
  lapply(analysis_index_cubes, function(cube){
    ...
    time_trial_elec <- ravetools::collapse(cube_data, keep = c(2, 3, 4), average = FALSE)
    freq_trial_elec <- ravetools::collapse(cube_data, keep = c(1, 3, 4), average = FALSE)


    re$freq_elec <- ravetools::collapse(freq_trial_elec, keep = c(1, 3), average = FALSE) / prod(dm[c(2, 3)])
    re$trial_elec <- ravetools::collapse(freq_trial_elec, keep = c(2, 3), average = FALSE) / prod(dm[c(1, 2)])
    re$time_elec <- ravetools::collapse(time_trial_elec, keep = c(1, 3), average = FALSE) / prod(dm[c(1, 3)])

    re$freq_time <- ravetools::collapse(cube_data, keep = c(1, 2), average = FALSE) / prod(dm[c(3, 4)])
    re$freq_trial <- ravetools::collapse(freq_trial_elec, keep = c(1, 2), average = FALSE) / prod(dm[c(2, 4)])
    re$time_trial <- ravetools::collapse(time_trial_elec, keep = c(1, 2), average = FALSE) / prod(dm[c(1, 4)])
...
collapse_power.FileArray <- function(x, analysis_index_cubes){
  ...
  fun <- function(e){
    ...
    for(ii in seq_along(analysis_index_cubes)){
      ...
      if(e %in% cube$Electrode){
        ...
        freq_time_elec <- ravetools::collapse(cube_data, keep = c(1, 2), average = FALSE)
        time_trial_elec <- ravetools::collapse(cube_data, keep = c(2, 3), average = FALSE)
        freq_trial_elec <- ravetools::collapse(cube_data, keep = c(1, 3), average = FALSE)
  ...
  structure(
    lapply(seq_along(analysis_index_cubes), function(ii){
      ...
      re$freq_time <- ravetools::collapse(re$freq_time_elec, keep = c(1, 2), average = FALSE) / prod(dm[c(3, 4)])
      re$freq_trial <- ravetools::collapse(re$freq_trial_elec, keep = c(1, 2), average = FALSE) / prod(dm[c(2, 4)])
      re$time_trial <- ravetools::collapse(re$time_trial_elec, keep = c(1, 2), average = FALSE) / prod(dm[c(2, 3)])

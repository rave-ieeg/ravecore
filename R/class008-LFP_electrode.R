#' @title Definitions of electrode with local field potential signal type
...
LFP_electrode <- R6::R6Class(
  ...
  public = list(
    ...
    `@unmarshal` = function(object) {
      ...
      if (length(object$data$reference) > 0 && !identical(object$data$reference, "noref")) {
        re$set_reference(reference = object$data$reference)
      }
      re
    },
    ...
    initialize = function(subject, number, quiet = FALSE) {
      ...
      if (!all(has_power, has_phase, has_volt)) {
        if (!quiet) {
          ravepipeline::logger("LFP channel {self$number} is missing {ifelse(has_power, '', 'power')}{ifelse(has_phase, '', ', phase')}{ifelse(has_volt, '', ', voltage')} data\n", level = "warning", use_glue = TRUE)
        }
      }
    },

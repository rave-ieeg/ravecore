...
generate_reference <- function(subject, electrodes) {
  ...
  electrode_text <- deparse_svec(electrodes)

  ref_cache_path <- file.path(subject$reference_path,
                              sprintf("ref_%s", electrode_text))

  ref_signals <- ravepipeline::lapply_jobs(blocks, function(block) {
    ravecore <- asNamespace("ravecore")

    ref_signal <- 0
    subject_inst <- ravecore$restore_subject_instance(subject_id, strict = FALSE)

    for (e in electrodes) {
      inst <- ravecore$LFP_electrode$new(subject = subject_inst,
                                         number = e,
                                         quiet = TRUE)
      if (has_wavelet) {
    ...
    ravecore$dir_create2(dirname(sarray_path))
    sarray <- filearray::filearray_create(
      filebase = sarray_path,
      dimension = c(length(ref_signal), 1L),
      type = "double"
    )
    sarray[] <- ref_signal
    ...
    if (has_wavelet && is.list(wavelet_params)) {
      ...
      coef <- 0
      for (e in electrodes) {
        inst <- ravecore$LFP_electrode$new(subject = subject_inst, number = e)
        power <- ieegio::io_read_h5(inst$power_file, sprintf("raw/power/%s", block), ram = TRUE)
        phase <- ieegio::io_read_h5(inst$phase_file, sprintf("raw/phase/%s", block), ram = TRUE)
    ...
    list(voltage = sarray, wavelet = warray)
  ...
  for(block in blocks){
    ...
    # voltage
    ieegio::io_write_h5(as.vector(item$voltage[]),
            file = target_file,
    ...
    if(!is.null(item$wavelet)) {
      ...
      ieegio::io_write_h5(coef_data, file = target_file,

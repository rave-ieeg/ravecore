  ...
  if(!isTRUE(all(srates > 1))) {
    ...
    re$sample_rate_set <- validate_result_new(
      ...
      message = sprintf("electrodes %s have invalid sample rates: %s",
                        deparse_svec(invalids), deparse1(srates[invalids])),
      .verbose = verbose
    )
  } else {
    re$sample_rate_set <- validate_result_new(
  ...
  if(!length(wavelet_electrodes)) {
    ...
    re$has_wavelet <- validate_result_new(
      ...
      description = sprintf(
        "Subject [%s] whether time-frequency decomposition has been applied to all LFP channels", subject$subject_id),
      message = sprintf(
        "please apply time-frequency decomposition (e.g. Wavelet) to the following electrode channels %s", deparse_svec(invalids)),
      .verbose = verbose
    )
  } else {
    re$has_wavelet <- validate_result_new(
...
validate_subject_voltage <- function(subject, version = 2, verbose = TRUE, other_checks = NULL) {
    ...
    expr = {
      ...
      signal_lengths <- ravepipeline::lapply_jobs(
        ...
        function(ii){
          ...
          h5names <- tryCatch({
            gsub("^/", "", ieegio::io_h5_names(pre_elec))
          }, error = function(e) {
            NULL
          })
          if(!length(h5names)) {
  ...
  re$voltage_data <- validate_from_expression(
    ...
    expr = {
      ...
      lfp_length_valid <- ravepipeline::lapply_jobs(
        seq_len(nrow(preproc_tbl)),
        function(ii) {
          ...
          # data missing?
          data_names <- ieegio::io_h5_names(voltage_file)
      ...
      all_length_valid <- ravepipeline::lapply_jobs(
        ...
        function(ii) {
          ...
          # data missing?
          data_names <- ieegio::io_h5_names(pre_voltage_file)
          if (!is.character(data_names)) {
            return(c(FALSE, FALSE, FALSE))
          }
          data_names <- gsub("^/", "", data_names)
          ...
          c(isTRUE(all(raw_lens > 0)), isTRUE(all(raw_lens == signal_length)), isTRUE(all(notch_lens == signal_length)))
        },
        .globals = list(
          preproc_tbl = preproc_tbl,
          blocks = blocks,

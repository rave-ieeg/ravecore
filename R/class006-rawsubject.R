...
RAVEPreprocessSettings <- R6::R6Class(
  ...
  public = list(
    ...
    `@marshal` = function(...) {
      list(
        namespace = "ravecore",
        r6_generator = "RAVEPreprocessSettings",
        data = list(
    ...
    `@unmarshal` = function(object, ...) {
      ...
      stopifnot(identical(object$r6_generator, "RAVEPreprocessSettings"))
      subject <- restore_subject_instance(object$data$subject, strict = FALSE)
      pretool <- subject$preprocess_settings
      pretool$read_only <- object$data$read_only
      list_to_fastmap2(object$data$data, pretool$data)
      return(pretool)
    },
      ...
      switch (
        ...
        "bids" = {
          ...
          query <- bidsr::query_bids(bids_subject, list(
            storage = "raw",
            sidecars = FALSE,
            data_types = c("ieeg", "eeg")
          ))
          query <- query[!query$extension %in% c("tsv", "csv"), ]

          block_names <- block_names_from_bids_entities(query$parsed)
...
#' @noRd
#' @param files `bidsr` parsed file names
block_names_from_bids_entities <- function(files) {
  # files <- query$parsed

  nms <- c("sub", "ses", "task", "run")

  blocks <- lapply(files, function(file) {
    nms <- nms[nms %in% names(file@entities)]
    entities <- lapply(file@entities[nms], format)
    paste(unlist(entities), collapse = "_")
  })
